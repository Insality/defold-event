-- Queue event module, the trigger events will wait for
-- their subscribers. So the subscribers and callbacks can be inited after the event happened
-- After processsing of event, the callback will be removed from the queue
-- Useful, if you don't know the order at initialization step
-- and want to make sure that the event will be processed
...
---Triggers a specified event and processes the queue of callbacks associated with it.
...
function M.trigger(event_name, callback, ...)
