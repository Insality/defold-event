return function()
	describe("Defold Event Promise", function()
		...
		it("Promise.append with promise waits for inner to complete", function()
			...
			assert(inner.value == "done")
			assert(pipeline:tail():is_resolved())
			assert(pipeline:tail().value == "done")
		end)

		it("Promise.append with promise does not resolve inner from upstream", function()
			local pipeline = promise.create()
			local inner = promise.create(function(resolve)
				resolve("async")
			end)
			pipeline:append(inner)
			pipeline:resolve("upstream")
			assert(inner:is_resolved())
			assert(inner.value == "async")
			assert(pipeline:tail():is_resolved())
			assert(pipeline:tail().value == "async")
		end)

		it("Promise.append with promise tail rejects when inner rejects", function()
			local pipeline = promise.create()
			local inner = promise.create()
			...
			inner:reject("step_failed")
			assert(pipeline:tail():is_rejected())
			assert(pipeline:tail().value == "step_failed")
		...
		describe("Promise cancellation", function()
			...
			it("Promise append with inner promise inherits cancel cleanup", function()
				...
				assert(pipeline:is_resolved())
				assert(pipeline:tail():is_rejected())
				assert(pipeline:tail():is_cancelled())
			end)
